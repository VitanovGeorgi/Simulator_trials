...
class PFEDUCB_v2(object):
    def __init__(self,
        ...
        self.M = cohorts[0].means.shape[0]
        self.K = cohorts[0].means.shape[1]
        self.local_means = cohorts[0].means
        ...
        self.clients = [
            client(index = i, cohort_id=self.cohorts[0].id, tstart = self.cohorts[0].t_0, thorizon = self.cohorts[0].T_c, narms=self.K, nclients = self.M, palpha = self.alpha, fp=fun_fp) for i in range(self.M)
        ]
    ...
    def simulate_single_step(self, plays):
        ...
        local_rewards = np.array([local_rews[i,plays[i]] for i in range(len(self.clients))])
        global_rewards = np.array([global_rews[plays[i]] for i in range(len(self.clients))])
    ...
    def simulate(self):
        ...
        for t in range(self.T):
            ...
            # plays = np.zeros(sum([cohort.means.shape[0] for cohort in self.cohorts if (cohort.t_0 >= t) and (cohort.T_c <= t)]))
            plays_list = []
            active_cohorts = []
            clients_aux = []
            
            for cohort in self.cohorts:
                if t in range(cohort.t_0, cohort.T_c):
                    active_cohorts.append(cohort)
                    # plays_list.append(cohort.means.shape[0])
            
            plays = np.zeros(sum([cohort.means.shape[0] for cohort in active_cohorts]))

            active_cohort_ids = [cohort.id for cohort in active_cohorts]
            for _client in self.clients:
                if _client.cohort_id in active_cohort_ids:
                    clients_aux.append(_client)
            # set of cohorts that have been added at time t
            unassigned_cohort_ids = list(set(active_cohort_ids) - set([_client.cohort_id for _client in clients_aux]))
            if len(unassigned_cohort_ids) > 0:
                for cohort in self.cohorts:
                    if cohort.id in unassigned_cohort_ids:
                        clients_aux += [
                            client(index = len(clients_aux) + i, \
                                cohort_id=cohort.id, tstart = cohort.t_0, \
                                    thorizon = cohort.T_c, narms=self.K, \
                                        nclients = cohort.means.shape[0], \
                                            palpha = self.alpha, fp=self.fun_fp) \
                                                for i in range(cohort.means.shape[0])
                        ] 
                        # update means for all new clients
                        self.local_means = np.concatenate((self.local_means, self.cohorts[1].means))
                x = 0

            self.clients = clients_aux
            self.M = len(self.clients)
            # tell the server that there are more clients
            self.server.nclients = self.M
            plays = [(int)(_client.play()) for _client in self.clients]
            ...
            for i in range(self.M):
                # when we add a new client, these need to be updated to reflect their local_rews
                self.clients[i].reward_update(plays[i], local_rews[i])  # update strategies of all player

            for i in range(self.M):
                # TODO: when updating the local_means, f_local_stat isn't updated on the right i
        ...
        top_mixed_means = np.zeros(len(self.clients))
        
        top_arms = np.zeros(len(self.clients), dtype=int)
        sub_gap = np.zeros(len(self.clients))
